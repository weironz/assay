...
`assay_ticket_get` 默认只返回文字与附件元数据。需要分析截图时，在同一次调用中传入：

```json
{ "id": "WO-20260905-0001", "includeImages": true }
```

MCP 会通过原有工单权限读取图片附件并返回可视觉识别的图片内容。单次最多 4 张、单张最多
10MB；图片较多时传 `imageIds` 指定最多 4 个附件 ID。非图片附件、超大图片和无权访问的
附件不会被读取。
