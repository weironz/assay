/**
 * English — the reference locale.
 *
 * The shape of this object is the source of truth for every other locale
 * (see `Resource` in ../index.ts), so adding a key here makes TypeScript
 * demand it everywhere else.
 */
const en = {
  common: {
    loading: 'Loading…',
    save: 'Save',
    cancel: 'Cancel',
    close: 'Close',
    confirm: 'Confirm',
    edit: 'Edit',
    view: 'View',
    delete: 'Delete',
    remove: 'Remove',
    back: 'Back',
    upload: 'Upload',
    submitting: 'Submitting…',
    processing: 'Processing…',
    notSpecified: 'Not specified',
    pleaseSelect: 'Please select',
    optional: '(optional)',
    empty: '—',
    actions: 'Actions',
    name: 'Name',
    email: 'Email',
    roles: 'Roles',
    status: 'Status',
    description: 'Description',
  },

  language: {
    label: 'Language',
    select: 'Select language',
  },

  theme: {
    toLight: 'Switch to light theme',
    toDark: 'Switch to dark theme',
  },

  brand: {
    subtitle: 'Ticket System',
    ariaHome: 'Greenstor Ticket System — home',
  },

  nav: {
    dashboard: 'Dashboard',
    tickets: 'Tickets',
    users: 'Users',
    queues: 'Queues',
    sla: 'SLA policy',
    profile: 'Settings',
    signOut: 'Sign out',
    toggleSidebar: 'Toggle sidebar',
    noAccess: 'You do not have permission to view this page.',
  },

  notifications: {
    title: 'Notifications',
    open: 'Notifications',
    markAllRead: 'Mark all read',
    empty: 'No notifications',
  },

  auth: {
    tabLogin: 'Sign in',
    tabRegister: 'Sign up',
    email: 'Email',
    name: 'Name',
    namePlaceholder: 'What should we call you?',
    password: 'Password',
    confirmPassword: 'Confirm password',
    rememberMe: 'Remember me',
    forgotPassword: 'Forgot password?',
    submitLogin: 'Sign in',
    submitRegister: 'Create account',
    resendVerification: 'Resend verification email',
    verifiedInfo: 'Your email has been verified. Please sign in.',
    verificationSent:
      'A verification email has been sent to {{email}}. Open the link in it to finish verification, then sign in.',
    verificationResent:
      'The verification email has been sent again to {{email}}. Please check your inbox.',
    errTooManyLogin: 'Too many sign-in attempts. Please try again later.',
    errEmailNotVerified:
      'Your email address is not verified yet. Please complete email verification first.',
    errInvalidCredentials: 'Incorrect email or password.',
    errSessionFailed: 'Could not establish a session.',
    errLoginFailed: 'Sign-in failed.',
    errTooManyRegister: 'Too many sign-up attempts. Please try again later.',
    errUserExists:
      'That email is already registered. Sign in, or reset your password.',
    errRegisterFailed: 'Sign-up failed.',
    errPasswordTooShort: 'Password must be at least 6 characters.',
    errPasswordMismatch: 'The two passwords do not match.',
    errSendFailed: 'Could not send the email. Please try again later.',
  },

  forgot: {
    title: 'Reset your password',
    intro: 'Enter your registered email and we will send you a reset link.',
    emailPlaceholder: 'Email',
    send: 'Send reset email',
    sending: 'Sending…',
    sent:
      'If {{email}} is registered, a reset link has been sent to it. Please check your inbox.',
    hint:
      'Nothing arrived? Check your spam folder, or try again later. The link is valid for 1 hour.',
    errTooMany: 'Too many requests. Please try again later.',
    errFailed: 'Could not send the email.',
    backToLogin: 'Back to sign in',
  },

  reset: {
    title: 'Set a new password',
    missingToken:
      'The reset token is missing. Please open this page from the link in your email.',
    done: '✓ Your password has been reset. Redirecting to sign in…',
    newPassword: 'New password (at least 6 characters)',
    confirmPassword: 'Confirm new password',
    submit: 'Reset password',
    errInvalidToken:
      'This link is invalid or has expired. Please request a new reset email.',
    errFailed: 'Could not reset the password.',
  },

  dashboard: {
    title: 'Dashboard',
    welcome: 'Welcome, {{name}} ({{roles}})',
    total: 'Total tickets',
    open: 'Open',
    done: 'Completed',
    myTodo: 'My queue',
    unassigned: 'Unassigned',
    overdue: 'Overdue',
    unread: 'Unread notifications',
    distribution: 'Tickets by status',
    groupOpen: 'In progress',
    groupClosed: 'Finished',
  },

  tickets: {
    title: 'Tickets',
    new: '+ New ticket',
    filterAll: 'All',
    saveView: '★ Save current filter',
    savePrompt: 'Save the current filter as a view. Name it:',
    deleteView: 'Delete view',
    searchPlaceholder: 'Search title or ticket number',
    titlePlaceholder: 'Search title',
    ticketNoPlaceholder: 'Search ticket number',
    allStatuses: 'All statuses',
    allPriorities: 'All priorities',
    allQueues: 'All queues',
    allAssignees: 'All assignees',
    allRequesters: 'All requesters',
    colTicketNo: 'Ticket',
    colTitle: 'Title',
    colCategory: 'Category',
    copyTicketNo: 'Copy ticket number',
    copied: 'Ticket number copied',
    colStatus: 'Status',
    colSla: 'SLA',
    colPriority: 'Priority',
    colAssignee: 'Assignee',
    colRequester: 'Requester',
    colCreatedAt: 'Created',
    empty: 'No tickets',
    confirmDelete: 'Delete ticket {{no}}?',
    pagination: '{{total}} total · page {{page}} of {{pages}}',
    prev: 'Previous',
    next: 'Next',
  },

  contact: {
    dialogTitle: 'Contact details',
    phone: 'Mobile',
    phonePlaceholder: 'Number we should call or text',
    callTime: 'Call me',
    smsTime: 'Text me',
    callNone: "Don't call",
    smsNone: 'No texts',
    emailAlerts: 'Email alerts',
    emailPlaceholder: 'name@company.com',
    addEmail: '+ Add email address',
    saveAsDefault: 'Use this for my future tickets',
    clear: 'Clear',
    errPhoneRequired: 'Enter a number, or close without saving.',
    summaryPhone: 'Mobile {{phone}}',
    summaryCall: 'Calls {{value}}',
    summarySms: 'Texts {{value}}',
    summaryEmails: 'Email x{{n}}',
    time: {
      ANY: 'any time',
      WEEKDAY_9_18: 'weekdays 9:00-18:00',
      WEEKDAY_9_22: 'weekdays 9:00-22:00',
      DAILY_9_22: 'daily 9:00-22:00',
    },
    position: {
      label: 'Role',
      TECH_LEAD: 'Engineering lead',
      OPS_LEAD: 'Operations lead',
      FINANCE: 'Finance',
      CEO: 'CEO',
      OTHER: 'Other',
    },
  },
  slaAdmin: {
    title: 'SLA policy',
    intro: 'Response and resolution targets are set per ticket type.',
    colType: 'Ticket type',
    colResponse: 'First response within',
    colResolve: 'Resolve within',
    minutes: '{{n}} min',
    unitMinute: 'Minutes',
    unitHour: 'Hours',
    unitDay: 'Days',
    unitLabel: 'unit',
    errPositive: 'Enter a valid duration of at least 1 minute.',
    errResponseGreater: 'Response target cannot be longer than the resolution target.',
    errSave: 'Could not save. Please try again.',
    rulesTitle: 'How the clock works',
    ruleStart: 'The clock starts when the ticket is created, not when someone picks it up.',
    ruleHold: 'Time spent on hold does not count — the clock resumes where it stopped.',
    ruleBreach: 'On a breach everyone involved is notified, and a resolution breach also raises the priority one step.',
    ruleScope: 'Changes apply to tickets created from now on; deadlines already running stay as they are.',
  },
  ticketNew: {
    title: 'New ticket',
    fieldTitle: 'Title',
    fieldBody: 'Description (rich text; Markdown shortcuts work: # heading, **bold**, - list)',
    bodyPlaceholder: 'Describe your problem — you can paste or drag images straight in',
    attachments: 'Attachments',
    addFile: 'Add attachment',
    attachmentHint: 'Supports {{exts}}. Up to {{mb}} MB per file, {{max}} files in total.',
    errFile: {
      ext: '{{name}}: this file type is not supported.',
      size: '{{name}}: larger than {{mb}} MB.',
      count: '{{name}}: skipped, {{max}} attachments is the limit.',
      upload: '{{name}}: upload failed, please try again.',
    },
    priority: 'Priority',
    type: 'Type',
    category: 'Category',
    queue: 'Queue',
    datacenter: 'Datacenter',
    cluster: 'Cluster',
    serialNumber: 'Serial number (SN)',
    serialNumberPlaceholder: 'SN from the device label — separate several with commas',
    submit: 'Create ticket',
    categoryCustom: 'Other (type your own)',
    categoryCustomPlaceholder: 'Name the category',
    contact: 'Contact',
    contactPlaceholder: 'Not set — add a way to reach you',
    errCategoryNameRequired: 'Please name the category.',
    errBodyRequired: 'Please enter a description.',
    errCreateFailed: 'Could not create the ticket.',
  },

  ticketDetail: {
    back: '← Back',
    selectAssignee: 'Select an assignee…',
    assign: 'Assign',
    internalTag: 'Internal note',
    internalCheckbox: 'Internal note (not visible to the requester)',
    replyPlaceholder: 'Write a reply… (Markdown shortcuts and images supported)',
    addInternalNote: 'Add internal note',
    reply: 'Reply',
    collaboration: 'Collaboration',
    collaborationHint: 'One owner drives SLA; teammates help without taking ownership.',
    primaryAssignee: 'Primary assignee',
    collaborators: 'Collaborators',
    followers: 'Followers',
    collaborator: 'Collaborator',
    follower: 'Follower',
    selectParticipant: 'Select a teammate…',
    participantRole: 'Collaboration role',
    addParticipant: 'Add to ticket',
    removeParticipant: 'Remove',
    mention: 'Mention',
    followOnlyHint: 'You are following this ticket. You will receive updates, but cannot reply.',
    metaStatus: 'Status',
    metaPriority: 'Priority',
    metaRequester: 'Requester',
    metaContact: 'Contact',
    metaAssignee: 'Assignee',
    metaQueue: 'Queue',
    metaType: 'Type',
    metaCategory: 'Category',
    metaDatacenter: 'Datacenter',
    metaCluster: 'Cluster',
    metaSerialNumber: 'Serial number',
    metaFirstResponse: 'First response',
    responseDue: 'due {{time}}',
    systemActor: 'System',
    metaSlaRemaining: 'SLA remaining',
    metaSlaDue: 'SLA due',
    metaCreatedAt: 'Created',
    unassigned: 'Unassigned',
    attachments: 'Attachments',
    addAttachment: '+ Upload',
    noAttachments: 'No attachments',
    history: 'Activity ({{n}})',
  },

  historyAction: {
    CREATE: 'created the ticket',
    ASSIGN: 'assigned it',
    TRANSITION: 'changed the status',
    MESSAGE: 'replied / added a note',
    UPDATE: 'edited the ticket',
    ATTACH: 'added an attachment',
  },

  users: {
    title: 'Users',
    emailPlaceholder: 'Email',
    namePlaceholder: 'Full name',
    passwordPlaceholder: 'Initial password (min. 6)',
    create: 'Create user',
    rolesLabel: 'Roles:',
    created: 'User created.',
    errCreateFailed: 'Could not create the user.',
    statusActive: 'Active',
    statusDisabled: 'Disabled',
    editRoles: 'Edit roles',
    disable: 'Disable',
    enable: 'Enable',
    resetPassword: 'Reset password',
    resetPrompt: 'Set a new password for “{{name}}” (at least 6 characters):',
    errPasswordTooShort: 'Password must be at least 6 characters.',
    passwordReset: 'The password has been reset.',
    errResetFailed: 'Could not reset the password.',
    errSaveFailed: 'Could not save.',
    editRolesTitle: 'Edit roles · {{name}}',
    atLeastOneRole: 'Select at least one role.',
    fixedRolesHint: 'Roles are fixed by the system. “Ticket observer” can view all tickets but cannot edit, comment, or manage users.',
  },

  queues: {
    title: 'Queues',
    colName: 'Name',
    namePlaceholder: 'Queue name',
    descriptionPlaceholder: 'Description (optional)',
    create: 'Create queue',
  },

  profile: {
    title: 'Settings',
    intro: 'Manage your account, sign-in security, and API access.',
    tabAccount: 'Account',
    tabSecurity: 'Security',
    tabTokens: 'API tokens',
    tokenCreateTitle: 'Create API token',
    tokenCreateHint: 'For Assay CLI, MCP, and automation. The full token is shown once only.',
    tokenName: 'Token name',
    tokenNamePlaceholder: 'e.g. My Codex workstation',
    tokenNameRequired: 'Enter a name for this token.',
    tokenExpiry: 'Expires',
    days: 'days',
    neverExpires: 'Never',
    tokenScopes: 'Permissions',
    tokenScopeHint: 'A token can only use permissions your account already has.',
    createToken: 'Create token',
    tokenCreateFailed: 'Could not create the token.',
    tokenLoadFailed: 'Could not load API tokens.',
    tokenRevealedTitle: 'Copy this API token now',
    tokenRevealedHint: 'For your security, it will not be shown again after this panel is closed.',
    copyToken: 'Copy',
    tokenSaved: 'I have copied it',
    tokenListTitle: 'Your API tokens',
    tokenListHint: 'Only the prefix is retained for identification. Revoke anything you no longer use.',
    refreshTokens: 'Refresh',
    tokenEmpty: 'No API tokens yet.',
    tokenLastUsed: 'Last used',
    tokenExpires: 'Expires',
    rotateToken: 'Rotate',
    revokeToken: 'Revoke',
    tokenRotateConfirm: 'Rotate this token? The current token stops working immediately.',
    tokenRevokeConfirm: 'Revoke this token? This cannot be undone.',
    tokenRotateFailed: 'Could not rotate the token.',
    tokenRevokeFailed: 'Could not revoke the token.',
    tokenStatus: { ACTIVE: 'Active', REVOKED: 'Revoked', EXPIRED: 'Expired' },
    avatarSection: 'Avatar and details',
    avatarAlt: 'Avatar',
    changeAvatar: 'Change avatar',
    avatarHint: 'PNG, JPG, GIF or WEBP, up to 2 MB.',
    avatarUpdated: 'Avatar updated.',
    errAvatarFailed: 'Could not upload the avatar.',
    nameLabel: 'Full name',
    errNameRequired: 'Name cannot be empty.',
    saved: 'Your details have been saved.',
    errSaveFailed: 'Could not save.',
    emailLabel: 'Email:',
    verified: 'Verified',
    unverified: 'Not verified',
    rolesLabel: 'Roles:',
    saveProfile: 'Save details',
    changePassword: 'Change password',
    currentPassword: 'Current password',
    newPassword: 'New password (at least 6 characters)',
    confirmPassword: 'Confirm new password',
    errPasswordTooShort: 'The new password must be at least 6 characters.',
    errPasswordMismatch: 'The two new passwords do not match.',
    errWrongPassword:
      'Could not change the password — your current password may be incorrect.',
    passwordChanged:
      'Password changed. Sessions on your other devices have been signed out.',
    errChangeFailed: 'Could not change the password.',
    confirmChange: 'Change password',
    deleteAccount: 'Delete account',
    deleteWarning:
      'Deleting removes your account and sign-in credentials permanently; this cannot be undone.',
    deleteWarning2:
      'Note: if the account has any tickets or replies, audit rules prevent deletion — ask an administrator to disable it instead.',
    deleteStart: 'Delete my account',
    deletePasswordPlaceholder: 'Enter your current password to confirm',
    deleteConfirm: 'Permanently delete',
    errDeleteFailed: 'Could not delete the account.',
  },

  editor: {
    bold: 'Bold',
    italic: 'Italic',
    heading1: 'Heading 1',
    heading2: 'Heading 2',
    bulletList: '• List',
    orderedList: '1. List',
    quote: 'Quote',
    codeBlock: 'Code',
    image: '🖼 Image',
    errUploadFailed: 'Image upload failed.',
  },

  share: {
    open: 'Share',
    title: 'Share ticket',
    intro: 'Create a read-only link for external discussion. Internal notes, contact details, email addresses, and regular attachments are not included.',
    createTitle: 'Create a new link',
    nameLabel: 'Link name',
    namePlaceholder: 'e.g. Customer review',
    expiryLabel: 'Expires',
    expiry1: 'In 1 day',
    expiry7: 'In 7 days',
    expiry30: 'In 30 days',
    expiry90: 'In 90 days',
    expiryNever: 'Never expires',
    create: 'Create share link',
    createFailed: 'Could not create the share link.',
    createdTitle: 'Share link created',
    createdHint: 'Copy it now. The full link is shown only once and cannot be recovered after this panel is closed.',
    linkLabel: 'Share link',
    copy: 'Copy',
    copied: 'Copied',
    copyFailed: 'Could not copy the link. Select and copy it manually.',
    existingTitle: 'Existing links',
    existingHint: 'For security, existing link addresses are not shown again. Revoke a link to stop access immediately.',
    empty: 'No share links yet.',
    expiresAt: 'Expires {{date}}',
    neverExpires: 'Never expires',
    revoke: 'Revoke',
    revokeConfirm: 'Revoke this share link? Anyone using it will lose access immediately.',
    revokeFailed: 'Could not revoke the share link.',
    status: { ACTIVE: 'Active', REVOKED: 'Revoked', EXPIRED: 'Expired' },
    publicBrand: 'Shared ticket',
    unavailableTitle: 'This shared ticket is unavailable',
    unavailableHint: 'The link may be incorrect, expired, or revoked by its owner.',
    readOnly: 'Read-only',
    metaType: 'Type',
    metaCategory: 'Category',
    metaDatacenter: 'Datacenter',
    metaCluster: 'Cluster',
    metaSerialNumber: 'Product serial number',
    metaCreatedAt: 'Created',
    conversation: 'Public conversation',
    conversationHint: 'This page only contains public replies. It cannot be used to modify the ticket.',
    noPublicMessages: 'There are no public messages in this ticket.',
    publicExpiresAt: 'This share link expires {{date}}.',
    publicNeverExpires: 'This share link does not expire unless it is revoked.',
  },

  sla: {
    title: 'SLA time remaining',
    overdue: 'Overdue by {{time}}',
    remaining: '{{time}} left',
    durMinutes: '{{m}} min',
    durHours: '{{h}} h',
    durHoursMinutes: '{{h}} h {{m}} min',
    durDaysHours: '{{d}} d {{h}} h',
    durDays: '{{d}} d',
    paused: 'On hold',
    pausedHint: 'The SLA clock is paused while the ticket is on hold.',
  },

  status: {
    NEW: 'New',
    ASSIGNED: 'Assigned',
    IN_PROGRESS: 'In Progress',
    PENDING: 'Pending',
    RESOLVED: 'Resolved',
    CLOSED: 'Closed',
    REOPENED: 'Reopened',
    CANCELLED: 'Cancelled',
  },

  priority: {
    LOW: 'Low',
    MEDIUM: 'Medium',
    HIGH: 'High',
    URGENT: 'Urgent',
  },

  ticketAction: {
    start: 'Start work',
    hold: 'Put on hold',
    resume: 'Resume work',
    resolve: 'Submit resolution',
    close: 'Accept & close',
    reopen: 'Reopen',
    cancel: 'Cancel ticket',
  },
};

export default en;
